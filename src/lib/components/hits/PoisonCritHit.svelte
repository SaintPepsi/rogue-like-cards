<script lang="ts">
	...
	onMount(() => playHitSound('hit:poisonCrit', index));
...
<div
	class="hit poison-crit"
	style:left="calc(50% + {randomX}px)"
	style:top="calc(50% + {randomY}px)"
	style:animation-delay="{animationDelay}s"
>
	<span class="poison-crit-icon">💀</span>{formatNumber(damage)}
</div>
...
<style>
	.hit {
		position: absolute;
		font-weight: bold;
		animation: poison-crit-float 0.7s ease-out forwards;
		pointer-events: none;
		display: flex;
		opacity: 0;
